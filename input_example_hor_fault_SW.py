...
y = 16.*np.ones(1201)+seistools.rough.generate_profile(1201, 24, 10**-2, 8, h = 1, seed=100)
...
p.add_pert(fdfault.swparam('boxcar', x0 = 4., dx = 2., mus = 10000.),0)
...
traction_load = np.loadtxt("failure_stress.out") 
# add load perturbations
# parameters for the load pertubation
#p.add_load(fdfault.load('boxcar',0., 7., 2, 0., 0., 0., 5.6, 0.))


p.add_load(fdfault.load('boxcar',0., 7., 2, 0., 0., 0., 15.6, 0.))   # it was 11.6



#p.add_load(fdfault.load('boxcar',0., 3., 2, 0., 0., -40., 0.0, 0.))   # it was 11.6

#p.add_load(fdfault.load('boxcar', t0 = 0., x0 = 0., dx = 0., y0 = 0., dy = 0., sn = 0., s2 = 0., s3 =0.):
p.add_pert(fdfault.swparam('boxcar', x0 = 3., dx = 2., mus = 10000.),0)
...
p.add_output(fdfault.output('snfault','Sn',0, end_time-1, interval_time, 0, 1200, 2, 801, 801, 1, 0, 0, 1))
...
#########################
# Using getter to get some important info for my simulations
print('shear wave velocity', p.get_cfl () )
g=32.04
rho = 2.67
print('shear wave velocity', g/rho )
print( 'dx on the fault', x[2]-x[1] )
